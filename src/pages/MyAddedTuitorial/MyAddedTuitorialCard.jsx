...
const MyAddedTuitorialCard = ({ item , handleDelete}) => {
  const navigate = useNavigate();
    return (
        ...
        className=" rounded-xl shadow-md p-5 hover:shadow-lg transition border border-amber-700 dark:border-white"
      >
        <div className="flex flex-col md:flex-row gap-6 items-center">
          <img src={item.image} alt={item.language} className="w-32 h-32 rounded-lg object-cover" />
          <div className="flex-1 space-y-2">
            <h2 className="text-xl font-bold text-amber-800 dark:text-gray-100">{item.name}</h2>
